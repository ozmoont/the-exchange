...
4. **For new screens, push to formalise the design system.** Surface to the founder via Andy: "we have N pages now, the inline-style approach is starting to cost; here's a one-spec proposal to introduce design tokens via Tailwind."
...
If the existing inline-style approach genuinely won't support what the spec needs (e.g. responsive grid with specific breakpoint behaviour), surface it to the founder via Andy and propose the design-system spec.
